...
pygame.display.set_caption("Head Ball 2 - Special Edition")
...
class Player:
    ...
    def kick(self, keys, ball):
        ...
        if distance < self.radius + ball.radius + 25:
            if self.is_player_one:
                if keys[pygame.K_f]: # Raso terra
                    ball.vel_x = 16
                    ball.vel_y = -1
                elif keys[pygame.K_g]: # Pallonetto
                    ball.vel_x = 11
                    ball.vel_y = -14
            else:
                if keys[pygame.K_MINUS] or keys[pygame.K_KP_MINUS] or keys[45]: # Raso terra
                    ball.vel_x = -16
                    ball.vel_y = -1
                elif keys[pygame.K_PERIOD] or keys[pygame.K_KP_PERIOD] or keys[46]: # Pallonetto
                    ball.vel_x = -11
                    ball.vel_y = -14
...
class Ball:
    def __init__(self, x, y):
        ...
        self.is_grounded = False
        self.trail = [] # Lista per salvare le posizioni precedenti (Scia)

    def update(self):
        # Gestione della scia (salva le ultime 8 posizioni)
    ...
    def check_collision_player(self, player):
        ...
        if distance < min_dist:
            ...
            speed = max(math.hypot(self.vel_x, self.vel_y) * self.elasticity, 5) 
    ...
    def draw(self, surface):
        # Disegna la scia (Chicca 1)
        speed = math.hypot(self.vel_x, self.vel_y)
        if speed > 5: # Mostra la scia solo se la palla si muove velocemente
            for i, pos in enumerate(self.trail):
                # Più la posizione è vecchia, più il cerchio è piccolo e trasparente
                alpha = int((i / len(self.trail)) * 150)
                trail_color = (200, 200, 200)
                radius = int(self.radius * (i / len(self.trail)))
                if radius > 0:
                    # Pygame gestisce la trasparenza tramite superfici secondarie
...
def main():
    ...
    # Gestione Timer (Chicca 2)
    match_duration = 90 # 90 secondi come le partite reali
    ...
    while running:
        ...
        if not game_over:
            ...
            # Controllo GOAL con attivazione Flash (Chicca 3)
            if ball.y > 320:
                is_goal = False
                ...
                if is_goal:
                    goal_flash_timer = 30 # Mantiene l'effetto per 30 frame (~0.5 secondi)
                    ball = Ball(WIDTH // 2, 200)
...
BLUE = (30, 144, 255)
RED = (220, 20, 60)
BALL_COLOR = (245, 245, 245) # Bianco leggermente sporco per far risaltare le pezze
BLACK = (20, 20, 20)          # Colore delle sagome da calcio
GOAL_COLOR = (200, 200, 200)
GOLD = (255, 215, 0)

screen = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("Head Ball 2 - Soccer Ball Edition")
clock = pygame.time.Clock()
...
class Ball:
    ...
    def draw(self, surface):
        ...
        if speed > 5: 
            for i, pos in enumerate(self.trail):
                alpha = int((i / len(self.trail)) * 130)
                radius = int(self.radius * (i / len(self.trail)))
                if radius > 0:
                    trail_surf = pygame.Surface((radius*2, radius*2), pygame.SRCALPHA)
                    pygame.draw.circle(trail_surf, (180, 180, 180, alpha), (radius, radius), radius)
                    surface.blit(trail_surf, (int(pos[0] - radius), int(pos[1] - radius)))

        # 2. Base bianca della palla
        pygame.draw.circle(surface, BALL_COLOR, (int(self.x), int(self.y)), self.radius)

        # 3. --- DISEGNO SAGOME DA CALCIO (PENTAGONI E LINEE) ---
        # Centro del pentagono centrale
        cx, cy = int(self.x), int(self.y)
        
        # Generiamo i 5 punti del pentagono centrale (raggio circa il 30% della palla)
        p_radius = self.radius * 0.32
        pentagon_points = []
        for i in range(5):
            angle = math.radians(i * 72 - 18) # Sfasamento per orientarlo dritto
            px = cx + p_radius * math.cos(angle)
            py = cy + p_radius * math.sin(angle)
            pentagon_points.append((px, py))
        
        # Disegna il pentagono centrale nero
        pygame.draw.polygon(surface, BLACK, pentagon_points)

        # Disegna le cuciture esterne (linee che partono dal centro e vanno verso il bordo)
        for i in range(5):
            angle = math.radians(i * 72 - 18)
            # Linea interna al pentagono verso l'esterno
            start_x = cx + p_radius * math.cos(angle)
            start_y = cy + p_radius * math.sin(angle)
            
            # Arriva quasi al bordo della palla
            ...
            # Piccoli triangoli neri sul bordo per simulare le pezze laterali tagliate dalla prospettiva
            edge_angle = angle + math.radians(36) # Ruotato in mezzo alle linee
            bx = cx + self.radius * math.cos(edge_angle)
            by = cy + self.radius * math.sin(edge_angle)
            
            b_in_x = cx + (self.radius * 0.78) * math.cos(edge_angle)
            b_in_y = cy + (self.radius * 0.78) * math.sin(edge_angle)
            
            # Disegna un piccolo arco/linea di cucitura laterale
            pygame.draw.circle(surface, BLACK, (int(b_in_x), int(b_in_y)), 3)

        # 4. Bordo circolare esterno per pulire le sagome
