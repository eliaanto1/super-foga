...
screen = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("Head Ball 2 - Low Ball Kickoff")
clock = pygame.time.Clock()
...
class Player:
    ...
    def move(self, keys):
        ...
        # Collisione Traverse Giocatori (Porte a 60 e 940)
    ...
    while running:
        ...
        if not game_over:
            ...
            if ball.y > 325:
                ...
                if is_goal:
                    goal_flash_timer = 30 
                    # MODIFICATO: la coordinata Y iniziale passa da 220 a 300 (molto più in basso)
                    ball = Ball(WIDTH // 2, 300, is_kickoff=True)
                    ball.vel_x = kickoff_direction
                    # Spinta verso l'alto calibrata a -2 per evitare sbalzi eccessivi sul soffitto
                    ball.vel_y = -2 
                    player1.x, player1.y = 200, 400
                    player2.x, player2.y = 800, 400
        else:
